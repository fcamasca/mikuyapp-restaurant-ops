...
| E1-R24 | La primera pantalla de `ADMINISTRADOR` se llamará **Inicio** y resumirá exclusivamente el local actual. En este orden mostrará: KPI de ventas netas, pedidos pagados, ticket promedio y descuentos autorizados; descuentos pendientes y cierres con diferencia no leídos; `Flujo actual de pedidos`; ventas por `EFECTIVO`, `YAPE`, `PLIN` y `TARJETA`; y estado operativo de la caja. `Flujo actual de pedidos` será un snapshot de sólo consulta de pedidos abiertos en `ENVIADO`, `RECIBIDO_COCINA`, `EN_PREPARACION` y `LISTO`, agrupados como `Por recibir`, `En preparación` y `Listos para entregar`, con cantidad, mayor espera actual, espera promedio actual, mesas y detalle, sin acciones. La navegación ADMIN incluirá **OPERACIÓN** → `Pedidos`, `Pendientes por aprobar`; **REPORTES** → `Caja`, `Ventas`; y **CONFIGURACIÓN** → `Carta`, `Mesas`, `Usuarios`. `Pedidos` consultará la operación actual del mismo local y expondrá la anulación directa definida en R11–R12; `Pendientes por aprobar` contendrá sólo solicitudes que requieren decisión administrativa, actualmente descuentos. Los cierres con diferencia permanecerán informativos, sin aprobación. No incluirá selector/consolidación multilocal ni persistencia analítica, históricos, tendencias, SLA, rankings, productividad o productos más vendidos. | Must |
| E1-R25 | **(Correctivo posterior al cierre, 22/09/2026).** Ningún conflicto funcional de aplicación (regla de negocio no transitoria: sesión de caja cerrada, descuento ya solicitado/decidido, pedido ya anulado o sin saldo, mesa/pedido ya no disponibles para la operación, etc.) se representará con el SQLSTATE `40001` (`serialization_failure`), reservado exclusivamente para una eventual condición nativa de concurrencia de PostgreSQL (hoy inexistente: el esquema no usa aislamiento `SERIALIZABLE`). Los conflictos funcionales usarán el código `PT409`, reconocido por PostgREST como HTTP 409 sin disparar su reintento automático de transacción. El cambio no altera reglas de negocio, mensajes ni contratos de retorno existentes. | Must |
...
- **(Correctivo E1-R25).** Los errores de conflicto funcional (no transitorios) usan `errcode='PT409'`; `40001` queda reservado para una eventual condición nativa de serialización de PostgreSQL y no se genera manualmente en ninguna RPC ni trigger vigente.
...
La matriz detallada se mantiene en `test-plan.md`. Agrupación: R01–R08 → D02–D05, T02–T05, TP01–TP18; R09–R12 → D06–D07, T06–T07/T17, TP19–TP30/TP59–TP64 aplicables; R13–R18 → D08–D09, T08–T10, TP31–TP48; R19–R20 → D10, T11, TP49–TP55; R21–R22 → D11–D12, T12–T14, TP56–TP64; R23 → D02–D05/D10–D13, T15, TP02/TP06/TP10/TP17–TP18/TP51–TP52/TP55/TP59–TP60/TP62; R24 → D11/D16, T16–T17, TP59–TP64 aplicables.
