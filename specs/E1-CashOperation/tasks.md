...
**E1-T01–E1-T17 están completadas.** TP62, TP63 y TP64 quedaron aprobadas humanamente. E1 fue aprobada formalmente y queda **CERRADA** desde el 21/09/2026. La impresión de comandas se trasladó a Evolución 7 y no forma parte de estas tareas.

**E1-T18 (correctivo post-cierre, 22/09/2026)** no reabre ni amplía el alcance funcional cerrado: corrige exclusivamente la convención interna de códigos de error identificada durante una prueba de capacidad en DEV, sin cambiar reglas de negocio ni mensajes.
...
| E1-T14 | **COMPLETADA — validación humana y aceptación.** TP62, TP63 y TP64 aprobadas; evidencia documentada y aceptación explícita registrada. | Nuevo checkpoint T13, deployment Preview DEV | TP62 aprobado; TP63 aprobado; TP64 aprobado; aceptación formal creada. | Todos / TP62–TP64 | 4 h |
...
| E1-T16 | **COMPLETADA, VALIDADA Y APROBADA.** Inicio del local actual con sidebar/drawer, KPI, atención, flujo vivo, ventas por medio y caja; RPC mínima para los tres grupos y detalle de sólo consulta. | T12 y T15; historial/timestamps H3–H5 existentes | Orden definitivo del dashboard; clasificación y tiempos calculados por servidor; aislamiento local; detalle descendente; cero persistencia analítica; ADMIN sin mutaciones de Cocina/Mozo; responsive sin overflow. | R24 / TP59–TP64 aplicables | 12 h (`8 h` originales + `4 h` delta) |
| E1-T17 | **COMPLETADA, VALIDADA Y APROBADA.** Incorporar `Operación → Pedidos` a la navegación ADMIN y exponer la anulación directa ya implementada. | T07, lectura T10 y navegación T16 | Grilla del local con filtro inicial de anulables, conteo, última actualización y orden descendente; loading/vacío/error, estados anulable/bloqueado, motivo y confirmación; advertencia operativa; snapshot actualizado; responsive. Reutiliza `anular_pedido_supervisado` y amplía aditivamente la salida de `rpc_obtener_pedidos_operacion_admin()` sin cambiar reglas. La migración `20260921000400` fue aplicada manualmente en DEV; local y DEV quedaron alineados, sin despliegue a PROD. | R11–R12/R24 / TP26–TP30 y TP59–TP64 aplicables | 3 h |
| E1-T18 | **COMPLETADA, VALIDADA Y APROBADA (23/09/2026).** Auditó y corrigió la convención de `SQLSTATE` usada en `RAISE EXCEPTION`: los conflictos funcionales de aplicación dejaron de representarse con `40001` (que PostgREST 14 reintenta automáticamente como fallo transitorio) y pasaron a `PT409`; `40001` queda reservado para una eventual condición nativa de serialización de PostgreSQL. Causa raíz confirmada de un incidente de CPU alta en DEV originado por `rpc_solicitar_descuento_pedido`. Migración aplicada en DEV; campaña de capacidad post-correctivo (5/10/20/40 clientes concurrentes) ejecutada y cerrada sin escalar compute ni cambiar configuración de Supabase Free — ver `docs/E1_T18_CAPACITY_TEST_EVIDENCE.md` y D18. | T01–T17 (todas las funciones/triggers financieros y de flujo ya construidos) | Migración correctiva reemplazó sólo las versiones vigentes de las 17 funciones/triggers afectados; `cashierService.ts`, `kitchenRealtimeService.ts` y `waiterOrderService.ts` reconocen `PT409` conservando compatibilidad temporal con `40001`/`23505`; pruebas Node, `typecheck` y `build` en verde (build aprobado por el usuario en su entorno Windows habitual; el entorno Linux de verificación no pudo ejecutar `vite build` por ausencia de `@rolldown/binding-linux-x64-gnu`, limitación exclusivamente ambiental sin impacto en la validación — ver TP65); verificación de que ninguna función/trigger vigente de `public` conserva un `40001` manual; capacidad validada hasta 40 clientes concurrentes sin saturación de Supabase/PostgreSQL. Sin cambios de reglas de negocio, mensajes ni alcance funcional. | R25 / TP65 | 2 h |
...
6. T15 se construyó sobre apertura/cierre y auditoría ya validados; agregó persistencia, lectura e interfaz de notificaciones sin cambiar las reglas financieras.
7. T16 se construyó sobre reportes T12, notificaciones T15 e `historial_estado`/timestamps existentes, sin crear eventos ni persistir métricas. T17 restituyó el acceso administrativo a pedidos. TP62–TP64 cerraron la validación humana.
8. T17 depende de la anulación T07, la lectura operacional T10 y la navegación T16. Incluyó la interfaz y una migración aditiva para ampliar la lectura operacional existente, sin cambiar las reglas de anulación; no altera `Pendientes por aprobar` ni `Flujo actual de pedidos`.
...
10. T18 es un correctivo transversal posterior al cierre formal de E1: depende de que T01–T17 ya existan construidos (corrige sus funciones/triggers vigentes) y no reordena ni reabre ninguna tarea previa.
