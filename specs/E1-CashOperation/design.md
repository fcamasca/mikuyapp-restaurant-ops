...
La ruta inicial de `ADMINISTRADOR` se presenta como **Inicio**, un resumen operativo del local derivado del contexto autenticado. No recibe ni permite seleccionar `local_id`; tampoco consolida sedes. En desktop usa sidebar persistente con esta arquitectura: `Inicio`; **OPERACIÓN** → `Pedidos`, `Pendientes por aprobar`; **REPORTES** → `Caja`, `Ventas`; **CONFIGURACIÓN** → `Carta`, `Mesas`, `Usuarios`. En tablet y móvil la misma jerarquía se conserva dentro de un drawer/hamburguesa accesible, con cierre al navegar, foco controlado, navegación por teclado y sin scroll horizontal.

`Operación → Pedidos` presenta los pedidos actuales del local en una grilla mínima `Mesa | Pedido | Estado | Cuenta | Acción`; `Cuenta` agrupa en una sola celda el total neto y saldo autoritativos, sin crear columnas separadas. Reutiliza `rpc_obtener_pedidos_operacion_admin()`, que deriva actor/local del contexto, exige `ADMINISTRADOR` y devuelve mesa, estado, existencia/conteo de pagos, totales y `ultima_actualizacion_en` autoritativos. Esta marca es el mayor instante entre creación/modificación del pedido y su última transición registrada; la RPC ordena de forma descendente por ella. La vista activa inicialmente `Sólo anulables`, permite desactivarlo para consultar todos, muestra el conteo visible/total y presenta la fecha/hora sin segundos dentro de Estado. Un único control `Ordenar por` mantiene como opción inicial la última actualización más reciente y permite alternar a antigua, mesa, estado o saldo ascendente/descendente, sin agregar indicadores ambiguos a cada cabecera. Reutiliza `anular_pedido_supervisado(p_pedido_id, p_motivo, p_idempotency_key)` para la única mutación. La pantalla ofrece `Anular` sólo para estados de la matriz R11 sin pagos; muestra estado informativo sin acción para `PAGADO`, `ANULADO` o cualquier pedido con pago parcial/completo. Exige motivo, presenta confirmación inequívoca, añade advertencia de impacto para `EN_PREPARACION`, `LISTO` y `ENTREGADO`, bloquea doble envío y refresca el snapshot autoritativo tras el resultado. Sus estados incluyen loading, vacío, error/reintento, anulable y bloqueado por pago. No permite cobrar, modificar estados de Cocina/Mozo ni cancelar productos individuales.

`Pendientes por aprobar` contiene únicamente solicitudes que necesitan una decisión del administrador, actualmente descuentos. La anulación directa no entra en esa bandeja. Los cierres con diferencia tampoco entran: aparecen en Inicio mientras su notificación de alerta permanezca no leída y en la campana como parte de las notificaciones recientes. `Caja` y `Ventas` son consultas/reportes administrativos; no exponen operación de cobro ni convierten al administrador en `CAJA`. `Carta`, `Mesas` y `Usuarios` conservan su función de configuración.

Inicio se compone, en este orden definitivo, de: (1) KPI de ventas netas, pedidos pagados, ticket promedio y descuentos autorizados; (2) `Requiere tu atención`, limitado a descuentos pendientes y cierres con diferencia no leídos, con estado vacío **Todo en orden**; (3) `Flujo actual de pedidos`; (4) ventas por `EFECTIVO`, `YAPE`, `PLIN` y `TARJETA`, mediante importe y barra proporcional, con porcentaje sólo cuando no perjudique legibilidad; y (5) `Operación de caja`, con caja, estado, actor/hora de apertura, inicial, efectivo esperado, entradas, salidas y acceso a la consulta de Caja, conservando el bloque cuando no exista sesión activa.

`Flujo actual de pedidos` es un snapshot operacional en vivo y de sólo consulta. Clasifica exclusivamente pedidos actualmente abiertos del local autenticado en: `POR RECIBIR` = `ENVIADO`; `EN PREPARACIÓN` = `RECIBIDO_COCINA` o `EN_PREPARACION`; y `LISTOS PARA ENTREGAR` = `LISTO`. Cada tarjeta presenta cantidad actual, mayor espera, espera promedio, mesas involucradas y `Ver pedidos`. Al seleccionarla despliega debajo `Mesa | Pedido | Estado actual | Espera`, ordenado de mayor a menor espera. `ENTREGADO`, `PAGADO` y `ANULADO` quedan excluidos. El administrador no recibe acciones de Cocina o Mozo.

La espera se calcula con `now()` del servidor contra la entrada al grupo vigente. `Por recibir` empieza en la última entrada a `ENVIADO`; `En preparación` empieza al entrar en `RECIBIDO_COCINA`/`EN_PREPARACION` y no se reinicia durante `RECIBIDO_COCINA → EN_PREPARACION`; `Listos para entregar` empieza en la última entrada a `LISTO`. Mayor espera es el máximo actual y promedio es el promedio actual de los pedidos del grupo. El resultado no se persiste ni constituye una serie histórica.

Se incorpora una lectura PostgreSQL autorizada conceptual `rpc_obtener_flujo_actual_pedidos_admin()` sin `local_id` cliente. Obtiene actor/local mediante el contexto autenticado, exige `ADMINISTRADOR` activo, consulta `pedido`, `mesa`, `historial_estado` y timestamps existentes, y devuelve en un mismo contrato `servidor_ahora` y, por grupo, `codigo`, `cantidad`, `mayor_espera_segundos`, `promedio_espera_segundos`, mesas y un detalle con `pedido_id`, mesa, estado actual, `ingreso_grupo_en` y `espera_segundos`. Es `SECURITY DEFINER`, con owner controlado, `search_path = pg_catalog`, `PUBLIC`/`anon` revocados y `authenticated` limitado a ejecución; no amplía `SELECT` directo sobre tablas ni expone otro local. Todos los conteos y tiempos se resuelven en PostgreSQL con hora servidor.
...
E1 incorpora únicamente este snapshot actual no persistido. E8 conserva históricos, series, tendencias, comparaciones entre periodos/locales, metas/SLA, productividad, rankings, análisis de cuellos de botella, productos más vendidos y demás analítica. El diseño prioriza acciones vigentes y estado actual sobre densidad de indicadores. T16 y el acceso T17 `Operación → Pedidos` están construidos y validados técnicamente en frontend local, sin cambios de backend para T17. TP62 está aprobado humanamente; TP63 sigue en ejecución y TP64 permanece pendiente.
