...
export default function AdminHomePage({ context, onCash, onPending }: { readonly context: ValidatedProfileContext; readonly onCash: () => void; readonly onPending: () => void }) {
  ...
  return <main className="mx-auto max-w-7xl px-3 py-5 sm:px-6 sm:py-7">
    ...
    {loading ? <p aria-busy="true" className="mt-6 rounded-2xl bg-white p-6">Cargando Inicio…</p> : daily && <>
      ...
      <section className={`mt-5 rounded-2xl border border-stone-200 bg-white shadow-sm ${attentionCount === 1 ? "p-3 sm:p-4" : "p-4 sm:p-5"}`}><div className="flex items-center justify-between gap-3"><h2 className="text-xl font-bold">Requiere tu atención</h2><span className="rounded-full bg-stone-100 px-3 py-1 text-sm font-bold">{attentionCount}</span></div>
      ...
      <section className="mt-5 rounded-2xl border border-stone-200 bg-white p-4 shadow-sm sm:p-5"><h2 className="text-xl font-bold">Flujo actual de pedidos</h2><p className="mt-1 text-sm text-stone-600">Pedidos abiertos del local en este momento.</p>
        <div className="mt-4 grid gap-3 md:grid-cols-3">{orderFlow?.groups.map((group) => <button aria-expanded={group.count > 0 && selectedFlow === group.code} className={`min-w-0 rounded-2xl border p-4 text-left transition focus:outline-none focus:ring-4 focus:ring-emerald-100 ${group.count === 0 ? "cursor-default border-stone-200 bg-stone-50" : selectedFlow === group.code ? "border-emerald-600 bg-emerald-50" : "border-stone-200 hover:border-emerald-300"}`} disabled={group.count === 0} key={group.code} onClick={() => setSelectedFlow((current) => current === group.code ? null : group.code)} type="button"><span className="block text-xs font-bold tracking-wide text-stone-700">{group.name}</span><span className="mt-1 block text-xs text-stone-500">{flowSubtitle[group.code]}</span><span className="mt-4 block text-2xl font-bold">{group.count} {group.count === 1 ? "pedido" : "pedidos"}</span>{group.count === 0 ? <span className="mt-3 block text-sm text-stone-500">Sin pedidos pendientes</span> : <><span className="mt-3 flex justify-between gap-2 text-sm"><span className="text-stone-600">Mayor espera</span><b>{formatWait(group.maxWaitSeconds)}</b></span><span className="mt-1 flex justify-between gap-2 text-sm"><span className="text-stone-600">Promedio</span><b>{formatWait(group.averageWaitSeconds)}</b></span><span className="mt-3 block truncate text-sm font-semibold" title={group.tables.join(", ")}>Mesas {group.tables.join(", ")}</span><span className="mt-4 block text-sm font-bold text-emerald-800">{selectedFlow === group.code ? "Ocultar pedidos" : "Ver pedidos"} →</span></>}</button>)}</div>
