...
import { combineOrderObservation, createWaiterOrderService, type WaiterOrderDetail } from '../services/waiterOrderService'
...
export default function WaiterOrderPage({ context, orderId, onBack }: Props) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const [catalogResult, detailResult] = await Promise.all([catalog.getOperationalCatalog(context), orders.getOrderDetails(context, orderId)])
      if (cancelled) return
      setLoading(false)
  ...
  return <main className="min-h-screen overflow-x-hidden bg-stone-100 px-3 py-5 text-stone-900 sm:px-6 lg:px-8"><div className="mx-auto min-w-0 max-w-7xl">
    ...
    {loading ? <p aria-busy="true" className="mt-8">Cargando pedido y productos…</p> : <div className="mt-8 min-w-0">
      ...
      {mode === 'ORDER' && <section className="min-w-0 rounded-3xl border bg-white p-4 shadow-sm sm:p-6"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="text-xl font-bold">Pedido actual</h2><p className="mt-1 text-2xl font-bold text-emerald-900">Total {money.format(total)}</p></div><button className="min-h-12 rounded-xl bg-emerald-800 px-5 font-bold text-white" onClick={() => setMode('CATALOG')} type="button">+ Agregar productos</button></div>{details.length === 0 ? <p className="mt-4 text-stone-600">Aún no agregaste productos.</p> : <ul className="mt-6 grid gap-4">{orderedDetails.map((detail, index) => { const open = detail.estado === 'ABIERTO'; const detailBusy = busyDetails.includes(detail.id); const removing = detailBusy && confirmingRemoval === detail.id; const productName = names.get(detail.producto_id) ?? 'Producto'; return <Fragment key={detail.id}>{index === 0 && requestedDetails.length > 0 && <li className="list-none"><h3 className="text-lg font-bold text-stone-700">Ya solicitado</h3><p className="text-sm text-stone-500">Solo lectura</p></li>}{index === requestedDetails.length && openDetails.length > 0 && <li className="mt-2 list-none"><h3 className="text-lg font-bold text-emerald-800">Por enviar</h3><p className="text-sm text-stone-500">Puedes ajustar estos productos</p></li>}<li aria-busy={detailBusy} className="min-w-0 rounded-2xl border p-4"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><h3 className="break-words font-bold">{productName}</h3><p className="mt-1 text-sm text-stone-600">{money.format(detail.precio_unitario)}{!open && ` · Estado: ${detail.estado}`}</p></div>{open ? <button aria-label={`Retirar ${productName}`} className="grid h-11 w-11 shrink-0 place-items-center rounded-xl border border-stone-200 bg-transparent text-stone-500 transition-colors hover:border-rose-200 hover:bg-rose-50 hover:text-rose-700 active:bg-rose-100 disabled:opacity-50" disabled={detailBusy} onClick={() => setConfirmingRemoval(detail.id)} type="button"><svg aria-hidden="true" className="h-5 w-5" fill="none" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" viewBox="0 0 24 24"><path d="M3 6h18" /><path d="M8 6V4h8v2" /><path d="M19 6l-1 14H6L5 6" /><path d="M10 11v5" /><path d="M14 11v5" /></svg></button> : <strong className="shrink-0">× {detail.cantidad}</strong>}</div>{detail.observacion && <p className="mt-3 break-words text-sm text-stone-700">{detail.observacion}</p>}
          {open ? <><div className="mt-4 grid grid-cols-3 gap-2"><button aria-label="Disminuir cantidad" className="min-h-11 rounded-xl border text-xl font-bold disabled:opacity-40" disabled={detailBusy || detail.cantidad <= 1} onClick={() => { void quantity(detail, detail.cantidad - 1) }} type="button">−</button><span className="grid min-h-11 place-items-center rounded-xl bg-stone-100 font-bold">{detail.cantidad}</span><button aria-label="Aumentar cantidad" className="min-h-11 rounded-xl border text-xl font-bold" disabled={detailBusy} onClick={() => { void quantity(detail, detail.cantidad + 1) }} type="button">+</button></div>
