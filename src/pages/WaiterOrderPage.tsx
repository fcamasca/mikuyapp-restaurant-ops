...
export default function WaiterOrderPage({ context, orderId, isSigningOut, onBack, onSignOut }: Props) {
  ...
  async function deliverOrder() {
    if (!orders || deliveringRef.current || !canDeliver) return
    deliveringRef.current = true; setDelivering(true); setError(null); setDeliveryMessage(null)
    try {
      const result = await orders.deliverOrder(context, orderId)
      await reloadOrderSnapshot()
      setConfirmingDelivery(false)
      if (!result.ok) { setError(result.error.message); return }
      setDeliveryMessage('Pedido entregado. La mesa está pendiente de pago.')
    } finally {
  ...
  return <main className="min-h-screen overflow-x-hidden bg-stone-100 px-3 py-5 text-stone-900 sm:px-6 lg:px-8"><div className="mx-auto min-w-0 max-w-7xl">
    <header className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-sm font-semibold uppercase tracking-[0.18em] text-emerald-700">Pedido vigente</p><h1 className="mt-2 text-2xl font-bold sm:text-3xl">{review ? `${review.mesa.codigo} · ${review.mesa.nombre}` : `Pedido #${orderId}`}</h1><p className="mt-1 text-sm text-stone-600">Pedido #{orderId}{review && ` · ${review.estado} · Mesa ${review.mesa.estado}`}</p></div><div className="grid gap-2 sm:flex">{canDeliver && <button className="min-h-11 w-full rounded-xl bg-sky-800 px-4 py-3 font-bold text-white disabled:opacity-60 sm:w-auto" disabled={delivering} onClick={() => setConfirmingDelivery(true)} type="button">Entregar pedido</button>}{review?.estado === 'ABIERTO' && details.length === 0 && <button className="min-h-11 w-full rounded-xl border border-rose-300 bg-white px-4 py-3 font-semibold text-rose-800 disabled:opacity-60 sm:w-auto" disabled={releasing} onClick={() => setConfirmingRelease(true)} type="button">Liberar mesa</button>}<button className="min-h-11 w-full rounded-xl border bg-white px-4 py-3 font-semibold sm:w-auto" onClick={onBack} type="button">Volver a mesas</button><AuthenticatedUserMenu context={context} isSigningOut={isSigningOut} onSignOut={onSignOut} /></div></header>
    {confirmingDelivery && review && <div className="mt-5 rounded-2xl border border-sky-200 bg-sky-50 p-4" role="dialog" aria-modal="true" aria-labelledby="deliver-order-title"><p className="font-semibold text-sky-950" id="deliver-order-title">¿Entregar el pedido #{orderId} de {review.mesa.codigo}?</p><p className="mt-1 text-sm text-sky-900">La mesa quedará pendiente de pago y los detalles permanecerán LISTO.</p><div className="mt-4 grid grid-cols-2 gap-2 sm:max-w-sm"><button className="min-h-11 rounded-xl border border-stone-300 bg-white font-semibold" disabled={delivering} onClick={() => setConfirmingDelivery(false)} type="button">Cancelar</button><button aria-busy={delivering} className="min-h-11 rounded-xl bg-sky-800 font-semibold text-white disabled:opacity-60" disabled={delivering} onClick={() => { void deliverOrder() }} type="button">{delivering ? 'Entregando…' : 'Confirmar entrega'}</button></div></div>}
