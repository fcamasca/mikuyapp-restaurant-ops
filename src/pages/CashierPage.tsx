...
function DocumentView({ kind, localName, order, payment, onClose }: { readonly kind: 'PRECUENTA' | 'TICKET'; readonly localName: string; readonly order: CashierPendingOrder; readonly payment: PersistedPayment | null; readonly onClose: () => void }) {
  return <div aria-label={kind === 'PRECUENTA' ? 'Precuenta' : 'Ticket interno'} className="fixed inset-0 z-40 overflow-y-auto bg-stone-950/50 p-3 sm:p-8"><section className="mx-auto max-w-xl rounded-3xl bg-white p-5 shadow-xl sm:p-7"><div className="flex items-start justify-between gap-4"><div><p className="text-sm font-bold uppercase tracking-widest text-emerald-800">{localName}</p><h2 className="mt-2 text-2xl font-bold">{kind === 'PRECUENTA' ? 'Precuenta' : 'Ticket interno'}</h2><p className="mt-1 text-sm text-stone-600">Pedido #{order.orderId} · Mesa {order.tableCode}</p></div><button className="min-h-11 rounded-xl border border-stone-300 px-4 py-2 font-semibold" onClick={onClose} type="button">Cerrar</button></div><Consumption order={order} /><p className="mt-5 flex justify-between border-t-2 border-stone-900 pt-4 text-xl font-bold"><span>Total</span><span>{money.format(payment?.amount ?? order.total)}</span></p>{kind === 'PRECUENTA' ? <p className="mt-3 text-sm text-stone-500">Documento informativo · Pedido pendiente de pago</p> : payment && <div className="mt-4 rounded-xl bg-stone-100 p-3 text-sm"><p>Pago #{payment.paymentId} · {payment.method}</p><p className="mt-1">{dateTime.format(new Date(payment.paidAt))} · America/Lima</p></div>}</section></div>
}
