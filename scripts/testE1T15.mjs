...
docker(
  ["exec", "-i", container, "psql", "-XqAt", "-U", "supabase_admin", "-d", database,
    "-v", "ON_ERROR_STOP=1"],
  docker(["exec", container, "pg_dump", "-U", "supabase_admin", "-d", "postgres", "--schema-only", "--exclude-schema=public"]),
);
...
for (const name of [
  ...
  "e1_t11_auditoria_financiera",
]) {
  file(`supabase/tests/${name}.sql`);
  console.log(`PASS regresión afectada: ${name}`);
}

console.log("PASS perfil_usuario: la prueba SQL confirmó que ADMIN no lee directamente el perfil del actor");
