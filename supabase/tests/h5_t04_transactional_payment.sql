...
do $metadata$
declare v_definition text; v_e1_definition text; v_v2_definition text; v_cobro_definition text;
begin
  ...
  select pg_catalog.pg_get_functiondef('public.rpc_registrar_cobro_pedido(bigint,uuid,text,jsonb,uuid)'::regprocedure)
    into v_cobro_definition;
  ...
  if v_definition !~* 'rpc_registrar_pago_total_pedido'
    ...
    or (v_v2_definition is not null and v_v2_definition !~* 'rpc_registrar_cobro_pedido' and (
    ...
    or (v_v2_definition ~* 'rpc_registrar_cobro_pedido' and (
      v_cobro_definition !~* 'auth\.uid' or v_cobro_definition !~* 'obtener_contexto_autenticado'
      or v_cobro_definition !~* '''CAJA''' or v_cobro_definition !~* 'for update'
      or v_cobro_definition !~* 'fn_resolver_total_pedido'
      or v_cobro_definition !~* 'insert into public\.pago'
      or v_cobro_definition !~* '''ENTREGADO''' or v_cobro_definition !~* '''PAGADO'''))
