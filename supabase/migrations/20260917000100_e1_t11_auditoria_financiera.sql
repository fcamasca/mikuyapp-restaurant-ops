...
-- E1-T11 consolida el catálogo ya producido por T04–T09. No añade eventos,
-- autorizaciones genéricas ni modifica los contratos de escritura existentes.
alter table public.auditoria_caja
  add constraint ck_auditoria_caja_catalogo_e1 check (tipo in (
    'APERTURA','ENTRADA','SALIDA','SOLICITUD_DESCUENTO',
    'AUTORIZACION_DESCUENTO','RECHAZO_DESCUENTO','PAGO','ANULACION',
    'CIERRE','CIERRE_SUPERVISOR'
  ));

create index idx_auditoria_caja_sesion_creado_en
  on public.auditoria_caja(sesion_caja_id,creado_en,id)
  where sesion_caja_id is not null;
create index idx_auditoria_caja_pedido_creado_en
  on public.auditoria_caja(pedido_id,creado_en,id)
  where pedido_id is not null;

create function public.rpc_obtener_auditoria_financiera(
...
begin
  ...
  return query
    ...
    order by a.creado_en,a.id limit p_limite;
end $$;
...
comment on function public.rpc_obtener_auditoria_financiera(uuid,bigint,integer) is
  'CAJA/ADMINISTRADOR activo: reconstrucción append-only por sesión y/o pedido del mismo local. Catálogo E1 cerrado; sin escritura ni acceso cruzado.';
