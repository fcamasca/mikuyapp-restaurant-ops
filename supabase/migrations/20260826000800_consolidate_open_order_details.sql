...
begin
  ...
  -- Serializa únicamente altas del mismo producto en el mismo pedido.
  ...
  if v_detalle_id is not null then
    ...
    where detail_row.id = v_detalle_id
      and detail_row.estado = 'ABIERTO'
    returning detail_row.id, detail_row.pedido_id, detail_row.producto_id,
      detail_row.cantidad, detail_row.precio_unitario, detail_row.observacion, detail_row.estado;
    return;
  end if;
...
revoke all on function public.agregar_detalle_pedido(bigint, uuid, integer, text) from public;
revoke all on function public.agregar_detalle_pedido(bigint, uuid, integer, text) from anon;
